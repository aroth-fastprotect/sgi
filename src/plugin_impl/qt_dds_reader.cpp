...
QImage decodeDDSImage(const osg::Image * image)
{
	...
	QDataStream s(&imageData, QIODevice::ReadOnly);
	s.setByteOrder(QDataStream::LittleEndian);

	DDSHeader header;
	header.magic = ddsMagic;
	header.size = ddsSize;
	header.flags = DDSHeader::FlagHeight | DDSHeader::FlagWidth | DDSHeader::FlagDepth | DDSHeader::FlagPitch | DDSHeader::FlagMipmapCount;
	header.width = image->s();
	header.height = image->t();
	header.depth = image->r();
	header.pitchOrLinearSize = image->getRowStepInBytes();
	header.mipMapCount = image->getNumMipmapLevels();
	header.pixelFormat.size = pixelFormatSize;
	...
	case GL_COMPRESSED_RGB_S3TC_DXT1_EXT:
		...
		header.pixelFormat.flags = DDSPixelFormat::FlagRGB | DDSPixelFormat::FlagFourCC;
		format = FormatDXT1;
		break;
	case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT:
		...
		header.pixelFormat.flags = DDSPixelFormat::FlagRGBA;
		break;
	...
	QImage ret = readTexture(s, header, format, mipmapLevel);
	bool ok = s.status() == QDataStream::Ok && !ret.isNull();
